...
* ``height-above`` - диапазон блоков в Mainnet, через который нода приватного блокчейна создаёт подтверждающую анкоринг транзакцию с данными первой транзакции. Рекомендованное значения для параметра ``height-above`` это количество блоков, на которое возможен откат сети Mainnet.
* ``threshold`` - число блоков, которое отнимается от текущей высоты приватного блокчейна. В транзакцию для анкоринга, отправляемую в Mainnet, попадёт хеш блока, отсчитанного от текущего на число ``threshold``. Если устанавливается значение 0, то берётся хеш текущего блока. Рекомендуется устанавливать значение, близкое к величине блоков, на которую возможет откат приватного блокчейна.
